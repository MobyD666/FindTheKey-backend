...
class FindTheKey extends Extension
{
    constructor(config)
    {
        ...
        this.debugDB = this.isTrue(this.config.DEBUGDB) || (this.config.NODE_ENV === 'development'); 
    ...
    register(app,prefix)
    {
        ...
        app.post('/'+prefix+'api/keyholderlog',  async (req, res) => { await this.keyholderlog(req, res); });
    }
            ...
            case 'actions':
            {
                if (delayedEvent?.sessionId != undefined)
                {
                    ...
                    try
                    {
                        ...
                        await this.addLogMessage(delayedEvent.sessionId,{source:'Find the Key',message:'Processing delayed event(s)',icon:'clock-rotate-left',level:'major'});
    ...
    async StartGame(sessionId,userData,config)
    {
        ...
        await this.addLogMessage(sessionId,{source:'Find the Key',message:'Game started',icon:'play',level:'major'});
        ...
        await this.generateUnfairsAndBlocks(sessionId,userData,config,0);
        await this.storeUserData(sessionId,userData);
    ...
    async generateUnfairsAndBlocks(sessionId,userData,config,guessNumber,session=null)
    {
        ...
        if (this.isTrue(config?.unfairsenabled))
        {
            ...
            if (khLogMessage.result != '') await this.addLogMessage(sessionId,{source:'Find the Key',message:'Guess '+khLogMessage.guessNr+' setup: '+khLogMessage.result,icon:'gears',level:'major'});
        }
        else
    ...
    async processBasicInfo(session)
    {
        ...
        if (session.role=="keyholder") 
        {
            ...
            bi.gameLog=await this.getLogMessages(session.session.sessionId);
    ...
    async guessKey(req,res)
    {
        ...
        try
        {
            ...
            await this.addLogMessage(session.session.sessionId,{source:'Wearer',message:'Guess number '+(guessNr+1)+', wearer guessed key, provisional guess result is '+(guessOk?"correct":"incorrect"),icon:'key',level:'critical'});
            ...
            if (actionInfo.available)
             {
                ...
                if (blocks.length > 0)
                {
                    ...
                    await this.addLogMessage(session.session.sessionId,{source:'Find the Key',message:'Guess blocked because of blocker(s)',icon:'hand',level:'critical'});
                ...
                else
                {
                    ...
                    if (unfairs.includes('unabletoguess'))
                    {
                        ...
                        await this.addLogMessage(session.session.sessionId,{source:'Find the Key',message:'Guessed blocked by "Unableto guess" blocker',icon:'hand',level:'critical'});
                        response.guessProcessed=false;
                        //should increment the wrong guess counter????

                        userData=await this.generateUnfairsAndBlocks(session.session.sessionId,userData,config,userData.keysguessed,session.session);
                    ...
                    else
                    {
                        ...
                        if ((unfairs.includes('liecorrect')) && (guessOk))  // liecorrect unfair
                        {
                            if ((userData.otherKeys.length>0) && (config.keyspresented>1))
                            {
                                ...
                                await this.addLogMessage(session.session.sessionId,{source:'Find the Key',message:'"Lie correct blocker" activated',icon:'hand',level:'critical'});
                                guessOk=false;                            
                        ...
                        if (guessOk) 
                        {
                            ...
                            await this.addLogMessage(session.session.sessionId,{source:'Find the Key',message:'Guess recognized as correct, ending the game',icon:'key',level:'critical'});
                            userData.state= 'finished';
                            if (unfairs.includes('delayactions'))
                            {
                                await this.addLogMessage(session.session.sessionId,{source:'Find the Key',message:'Delay correct actions activated',icon:'clock-rotate-left',level:'major'});
                        ...
                        else 
                        {
                            ...
                            await this.addLogMessage(session.session.sessionId,{source:'Find the Key',message:'Guess recognized as incorrect',icon:'key',level:'major'});
                            ...
                            if (unfairs.includes('doubleactions')) 
                            {
                                ...
                                await this.addLogMessage(session.session.sessionId,{source:'Find the Key',message:'Double actions activated',icon:'calculator',level:'major'});
                            }
                            ...
                            await this.generateUnfairsAndBlocks(session.session.sessionId,userData,config,userData.keysguessed,session.session);
                            await this.storeUserData(session.session.sessionId,userData);
                        }
                    }
                 }
    ...
    //    actions.forEach(async action =>
            {
                ...
                switch (action.action) 
                {
                    case 'freeze': await this.freeze(sessionId);
                                   await this.addLogMessage(sessionId,{source:'Find the Key',message:'Lock freezed',icon:'snowflake',level:'major'});
                        break;
                    case 'unfreeze': await this.unfreeze(sessionId);
                                     await this.addLogMessage(sessionId,{source:'Find the Key',message:'Lock unfreezed',icon:'snowflake',level:'major'});
                        break;                        
                    case 'togglefreeze': await this.toggleFreeze(sessionId);
                        await this.addLogMessage(sessionId,{source:'Find the Key',message:'Lock freeze toggled',icon:'snowflake',level:'major'});
                       break;                        
                    case 'block': await this.setReasonsPreventingUnlocking(sessionId,'Must find a key');
                                  await this.addLogMessage(sessionId,{source:'Find the Key',message:'Unlock blocked',icon:'lock',level:'major'});                    
                        break;                                                
                    case 'unblock': await this.setReasonsPreventingUnlocking(sessionId,'');
                                  await this.addLogMessage(sessionId,{source:'Find the Key',message:'Unlock allowed',icon:'lock',level:'major'});                                        
                        break;  
                    case 'addtime': await this.addTime(sessionId,action.time);
                                    await this.addLogMessage(sessionId,{source:'Find the Key',message:'Time added '+this.formatTimeString(action.time),icon:'clock',level:'major'});
                        break;
                    case 'removetime': await this.removeTime(sessionId,action.time);
                                    await this.addLogMessage(sessionId,{source:'Find the Key',message:'Time removed '+this.formatTimeString(Math.abs(action.time)),icon:'clock',level:'major'});
                        break;
                    case 'resetkeys': 
                        {
                            if (this.chance(action.number))
                            {
                                ...
                                await this.addLogMessage(sessionId,{source:'Find the Key',message:'All (fakes and correct one) keys reset',icon:'key',level:'major'});
                            }
                    ...
                    case 'replaceguessedkey':
                            ...
                            if (action.action=='removeguessedkey')
                                {
                                    if  (postCount!=preCount) await this.addLogMessage(sessionId,{source:'Find the Key',message:'Guessed key removed',icon:'key',level:'major'});
                                }
                                else
                                    await this.addLogMessage(sessionId,{source:'Find the Key',message:'Guessed key replaced',icon:'key',level:'major'});
                    ...
                    case 'removefakekeys':
                            ...
                            await this.addLogMessage(sessionId,{source:'Find the Key',message:'Removed '+Math.abs(action.number)+' fake keys',icon:'key',level:'major'});
                        break;
                    case 'addfakekeys':
                            ...
                            await this.addLogMessage(sessionId,{source:'Find the Key',message:'Added '+action.number+' fake keys',icon:'key',level:'major'});
                        break;
                    case 'change': 
                        {
                            userData.otherKeys=this.shuffleInPlace(userData.otherKeys);
                            for (let i=0;i<action.number;i++) userData.otherKeys.shift();
                            await this.addLogMessage(sessionId,{source:'Find the Key',message:'Changed '+action.number+' fake keys',icon:'key',level:'major'});
                        }
                        break;                          
                    case 'changekey': 
                        {
                            if (this.chance(action.number))
                            {
                                ...
                                await this.addLogMessage(sessionId,{source:'Find the Key',message:'Changed the correct key',icon:'key',level:'major'});
                            }
                        }
                        break;  
                    case 'restartgame':
                        {
                            await this.addLogMessage(sessionId,{source:'Find the Key',message:'Game restarted because of action',icon:'arrows-spin',level:'major'});                            
                    ...
                    case 'pillory':
                            {
                                ...
                                if (reg.result)
                                {
                                    ...
                                    await this.addLogMessage(sessionId,{source:'Find the Key',message:'Wearer sent to pillory for '+this.formatTimeString(action.time),icon:'bulhorn',level:'major'});
                                }
                                else
                                {
    ...
    async restartGame(req,res)
    {
        ...
        try
        {
            ...
            if ( ((userData.state == 'finished') && (session.role=="wearer") && (config.disableplayerrestart != true) ) || ( (session.role=="keyholder") && (session?.session?.lock?.trusted===true) )  )
            {
                ...
                await this.addLogMessage(session.session.sessionId,{source:session.role,message:session.role+' restarted the game',icon:'arrows-spin',level:'major'});
    ...
    async unblock(req,res)
    {
        ...
        try
        {
            ...
            if (session.role=="keyholder")
            {
                ...
                await this.addLogMessage(session.session.sessionId,{source:'Keyholder',message:'Keyholder allowed unlocking',icon:'key',level:'critical'});
            }
            if (release != null) release();
    ...
    async keyholderlog(req,res)
    {
        ...
        try
        {
            ...
            if (!(cnt>0)) cnt=null;
            result=await this.getLogMessages(session.session.sessionId,cnt);
    ...
    async changeKey(req,res)
    {
        ...
        try
        {
            ...
            if (session.role=="keyholder")
            {
                ...
                userData=this.setNewKey(userData);
                await this.addLogMessage(session.session.sessionId,{source:session.role,message:session.role+' changed the correct key',icon:'key',level:'critical'});
                await this.storeUserData(session.session.sessionId,userData);
                if (session.role=="keyholder") 
                {
    ...
    async addFakeKeys(req,res)
    {
        ...
        try
        {
            ...
            {
                await this.addLogMessage(session.session.sessionId,{source:session.role,message:session.role+' '+((addCount>0)?'added':'removed')+' fake '+Math.abs(addCount)+' keys',icon:'key',level:'major'});
    ...
    async processActionLogEvent(e)
    {
        ...
        {
            await this.addLogMessage(session.session.sessionId,{source:'Find the Key',message:'Blockers removed: '+blocksRemoved.map( (u) => this.unfairName(u)).join(', ')+"\r\n",icon:'eraser',level:'major'});
            await this.storeUserData(session.session.sessionId,userData);
        }
    ...
    async addLogMessage(sessionId,msg)
    {
        ...
        await this.storeLogMessage(sessionId,msg);
        //userData.log.push(msg);
        return (msg);
    }

    async storeLogMessage(sessionId,msg)
    {
        try 
        {
            
            const profilename=sessionId+'-'+this.generateRandomIdentifier(8);
    ...
    async getLogMessages(sessionId,cnt=null)
    {
        let msgs=[];
        try 
        {
        ...
        {
            console.log(sessionId,'SQL ERROR getLogMessages error ',error);
        }
        return(msgs);
    }
